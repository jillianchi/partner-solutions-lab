  ...
  return (
    <div>
      ...
      <p className="text-sm mb-3" style={{ color: '#425466' }}>
        By default, Stripe automatically pays out connected accounts on a rolling basis. For TableOS, we configure <code className="text-xs px-1 py-0.5 rounded" style={{ backgroundColor: '#EEF2FF', color: '#533AFD' }}>payout_schedule.interval = manual</code> on the account at creation time and trigger payouts explicitly via the API.
      </p>

      <CodeBlock
        language="javascript"
        filename="server/routes/accounts.js (update account creation from Task 2.1)"
        code={`// Add payout schedule when creating the connected account
const account = await stripe.accounts.create({
  // ... other params from Task 2.1
  settings: {
    payouts: {
...
async function getAvailableBalance(stripeAccountId) {
  ...
  // Guard against zero-balance: Stripe rejects payouts below minimum amount
  if (!sgdBalance || sgdBalance.amount === 0) return 0;
  return sgdBalance.amount;
}`}
      />
